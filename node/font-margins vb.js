...
const SMOOTH_PCT   = parseFloat(process.argv[3] ?? "70") / 100;  // z.B. 0.70 = 70%
...
/**
 * Glättet ein Margin-Array so, dass benachbarte gültige Zonen maximal um
 * smoothPct × zoneHeight voneinander abweichen.
 *
 * Algorithmus (zwei Durchläufe: vorwärts + rückwärts):
 *   - Ungültige Zonen (-1) bleiben unverändert.
 *   - Ausgangspunkt für die Glättungsrichtung ist immer die Zone mit dem
 *     maximalen Ausdehnung des Zeichens (d.h. der MINIMALE Margin-Wert,
 *     da kleinerer Margin = Kontur reicht weiter zum Rand).
 *   - Jede Zone darf vom Vorgänger maximal um maxDelta abweichen.
 *   - Mehrere Durchläufe bis Konvergenz (max. zones Iterationen).
 *
...
function smoothMargins(arr, zoneHeight, smoothPct) {
  ...
  const n = arr.length;
  const out = arr.slice();   // Kopie

  // Mehrere Durchläufe bis keine Änderung mehr
  for (let pass = 0; pass < n; pass++) {
    let changed = false;

    // Vorwärtsdurchlauf: z=0 → z=n-1
    for (let z = 1; z < n; z++) {
      if (out[z] === -1 || out[z - 1] === -1) continue;
      const diff = out[z] - out[z - 1];
      if (Math.abs(diff) > maxDelta) {
        out[z] = round1(out[z - 1] + Math.sign(diff) * maxDelta);
        changed = true;
      }
    }

    // Rückwärtsdurchlauf: z=n-1 → z=0
    for (let z = n - 2; z >= 0; z--) {
      if (out[z] === -1 || out[z + 1] === -1) continue;
      const diff = out[z] - out[z + 1];
      if (Math.abs(diff) > maxDelta) {
