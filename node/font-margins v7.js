...
const BASELINE_UC1 = "O";   // Großbuchstaben & Lining-Ziffern → Mittelwert aus N+O und O+N
const BASELINE_UC2 = "O";
...
async function processFont(fontPath) {
  ...
  // UC-Basiswert: Mittelwert aus N+O und O+N
  const nUC1 = glyphCache[BASELINE_UC1];
  const nUC2 = glyphCache[BASELINE_UC2];
  const baseCalcUC1 = (nUC1 && nUC2) ? pairMean(nUC1.right, nUC2.left) : null;
  const baseCalcUC2 = (nUC1 && nUC2) ? pairMean(nUC2.right, nUC1.left) : null;

  let baseValueUC = null;
  let baseUCNote  = "";

  if (baseCalcUC1 && baseCalcUC2) {
    baseValueUC = (baseCalcUC1.mean + baseCalcUC2.mean) / 2;
    baseUCNote  = `Ø ${round1(baseValueUC)} (N+O: ${round1(baseCalcUC1.mean)}, O+N: ${round1(baseCalcUC2.mean)})`;
  } else if (baseCalcUC1) {
    baseValueUC = baseCalcUC1.mean;
    baseUCNote  = `Ø ${round1(baseValueUC)} (nur N+O)`;
  } else {
    // Fallback: UC-Basiswert = LC-Basiswert
    baseValueUC = baseCalcLC.mean;
    baseUCNote  = `Fallback auf LC-Basiswert (N oder O nicht gefunden)`;
  }
  ...
  console.log(`     Basiswert UC "${BASELINE_UC1}"+"${BASELINE_UC2}": ${baseUCNote}`);
  ...
  const pairsResult = {
    meta: {
      ...
      baseGlyphUC: `${BASELINE_UC1}+${BASELINE_UC2}`,
      description:
  ...
  for (const keyA of glyphKeys) {
    ...
    // Basiswert nach Klasse des LINKEN Zeichens bestimmen
    ...
    for (const keyB of glyphKeys) {
      ...
      //   pairMean > baseValue → zu weit auseinander → negativ (enger)
      //   pairMean < baseValue → zu eng              → positiv (weiter)
...
async function main() {
  ...
  for (const fontFile of fontFiles) {
    ...
    try {
      ...
      if (kerningLines) {
        const csvContent =
          `# Kerning | Font: ${baseName} | Zonen: ${ZONES}\n` +
          `# Basiswert LC (n+n): Ø${round1(baseValueLC)}  |  Basiswert UC (N+O): Ø${round1(baseValueUC)}\n` +
