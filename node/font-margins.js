...
Analyzes TTF/OTF fonts in ./fonts and outputs margin, pair-gap, and kerning data.

Usage:
  node font-margins.js [options]

Options:
  --zones=N          Number of height zones (default: 9)
  --smooth=N         Max neighbor-zone delta as % of zone height (default: 40)
  --round=N          Kerning rounding module, e.g. 46 means multiples of 46 (default: 1)
  --supersample=N    Subzone count per zone for edge averaging (default: 10)
  --mingap=N         Min gap floor as fraction of base value, e.g. 4 means 1/4 (default: 4)
  --help             Show detailed help and exit

Run without arguments to use all defaults.
...
DESCRIPTION
  Reads all TTF and OTF font files from ./fonts, analyzes glyph outlines using
  Bézier curve intersection, and produces three sets of output files:

    ./margins/<fontname>.json   Per-glyph left/right margin arrays across height zones
    ./pairs/<fontname>.json     Per-pair gap arrays (right_A[z] + left_B[z])
    ./kerning/<fontname>.csv    Kerning correction values relative to reference pair

USAGE
  node font-margins.js [options]

  All parameters are optional named flags. Positional arguments are not supported.

OPTIONS
  --zones=N
      Number of equal-height zones spanning the full em height (ascender to descender).
      Higher values give finer vertical resolution but increase processing time quadratically
      (due to pair combinations) and may produce more -1 (empty) zones.
      Default: 9

  --smooth=N
      Neighbor-zone smoothing threshold as a percentage of zone height.
      After measuring raw margins, adjacent zones are constrained so that no zone
      may deviate from its neighbor by more than (N/100 × zoneHeight) font units.
      Smoothing always anchors on the zone with the smallest margin value
      (= maximum glyph extent) and propagates outward from there.
      The anchor value itself is never modified.
      Set to 0 to disable smoothing entirely.
      Default: 40

  --round=N
      Kerning correction rounding module. The final kerning value is rounded
      to the nearest multiple of N using commercial (half-up) rounding.
      Example: --round=46 produces values like -92, -46, 0, 46, 92 ...
      Use --round=1 for no module rounding (still rounds to integer).
      Default: 1

  --supersample=N
      Subzone count per zone for edge anti-aliasing.
      Each zone is internally subdivided into N equal sub-zones. The margin for
      each sub-zone is computed independently, then averaged to produce the
      final zone margin. This prevents cliff artifacts at zone boundaries caused
      by curves that cross a boundary at an oblique angle.
      Example: --supersample=10 with --zones=9 internally computes 90 sub-zones.
      Higher values improve accuracy but increase processing time linearly.
      Default: 10

  --mingap=N
      Minimum gap floor divisor for kerning capping.
      After computing a correction value, the script ensures that the tightest
      zone gap after applying the correction never falls below (baseValue / N).
      Example: --mingap=4 means the floor is one-quarter of the reference gap.
               --mingap=2 means the floor is one-half (more conservative).
               --mingap=8 means the floor is one-eighth (more aggressive kerning).
      Default: 4
...
OUTPUT FORMAT
  margins/<fontname>.json
    {
      "meta": { "fontName", "upm", "zones", "zoneHeight", "yBottom", "yTop" },
      "glyphs": {
        "A": {
          "name": "A", "unicode": 65, "unicodeHex": "U+0041", "char": "A",
          "advanceWidth": 1234,
          "left":            [...],   // smoothed margins (used for kerning)
          "right":           [...],
          "leftUnweighted":  [...],   // raw supersampled margins (for debugging)
          "rightUnweighted": [...]
        }, ...
      }
    }
    Zone index 0 = bottom of em (near descender), last index = top (near ascender).
    Value -1 means no glyph contour was found in that zone.

  pairs/<fontname>.json
    { "meta": {...}, "pairs": { "A": { "V": [gap_z0, gap_z1, ...], ... }, ... } }
    gap[z] = right_A[z] + left_B[z], or -1 if either side is invalid in that zone.

  kerning/<fontname>.csv
    Format: LeftGlyph;RightGlyph;Correction (BaseClass z0:val,z1:val,...) [cap:...]
    Correction = round((baseValue - pairMean) / roundModule) * roundModule
    Negative = tighten spacing, Positive = open spacing.

BASELINE REFERENCES
  LC glyphs (lowercase + oldstyle figures):  reference pair is n+n
  UC glyphs (uppercase + lining figures):    reference pair is O+O
  Mixed pairs (one UC + one LC):             uses LC baseline

EXAMPLES
  node font-margins.js
      Run with all defaults (9 zones, 40% smoothing, no rounding, 10× supersampling, 1/4 mingap)

  node font-margins.js --zones=16 --smooth=40 --round=46 --supersample=10 --mingap=4
      16 zones, 40% smoothing, round to multiples of 46, 10× supersampling, 1/4 min gap

  node font-margins.js --zones=9 --smooth=0
      Disable smoothing, keep all other defaults

  node font-margins.js --help
      Show this help text
`.trim();
...
function getArg(name, defaultVal, parser) {
  const prefix = `--${name}=`;
  const found  = rawArgs.find(a => a.startsWith(prefix));
  if (!found) return defaultVal;
  const val = parser(found.slice(prefix.length));
  if (isNaN(val) || val < (parser === parseFloat ? 0 : 1)) {
    console.error(`Error: --${name} has invalid value "${found.slice(prefix.length)}"`);
    process.exit(1);
  }
  return val;
}

const ZONES       = getArg("zones",       9,   s => parseInt(s, 10));
const SMOOTH_PCT  = getArg("smooth",      40,  parseFloat) / 100;
const KERN_ROUND  = getArg("round",       1,   s => parseInt(s, 10));
const SUPERSAMPLE = getArg("supersample", 10,  s => parseInt(s, 10));
const MINGAP_DIV  = getArg("mingap",      4,   s => parseInt(s, 10));

// Check for unknown flags
for (const arg of rawArgs) {
  if (arg.startsWith("--") && arg !== "--help") {
    const key = arg.split("=")[0];
    if (!["--zones","--smooth","--round","--supersample","--mingap"].includes(key)) {
      console.error(`Error: Unknown option "${key}". Run --help for usage.`);
      process.exit(1);
    }
  }
}
...
// Baseline reference glyphs
const BASELINE_LC  = "n";   // lowercase & oldstyle figures  → n+n
const BASELINE_UC1 = "O";   // uppercase & lining figures    → O+O
...
function computeGlyphMargins(glyph, upm, zones, yBottom, yTop) {
  ...
  const zoneHeight   = (yTop - yBottom) / zones;
  const subHeight    = zoneHeight / SUPERSAMPLE;   // Höhe einer Subzone
  const rawPath      = glyph.getPath(0, 0, upm);

  // ── Rohwerte mit Supersampling ───────────────────────────────────────────────
  // Jede Zone wird in SUPERSAMPLE Subzonen unterteilt.
  // Pro Subzone wird xMin/xMax berechnet.
  // Der Zonenwert ist der Mittelwert der gültigen Subzonen-Margins.
  //
  // Kanteneffekt: Kurvenabschnitte, die eine Zonengrenze nur leicht berühren,
  // fließen proportional ein statt die gesamte Zone zu dominieren.

  // Alle Subzonen auf einmal berechnen (ZONES × SUPERSAMPLE Subzonen)
  const totalSubs  = zones * SUPERSAMPLE;
  const subRanges  = pathXRangesPerZone(rawPath, totalSubs, yBottom, yTop);

  const leftRaw = [], rightRaw = [];

  for (let z = 0; z < zones; z++) {
    const subStart = z * SUPERSAMPLE;
    let   sumLeft  = 0, sumRight = 0, count = 0;

    for (let s = 0; s < SUPERSAMPLE; s++) {
      const sub = subRanges[subStart + s];
      if (!sub) continue;                             // Subzone ohne Kontur → überspringen
      sumLeft  += sub.xMin;
      sumRight += advanceWidth - sub.xMax;
      count++;
    }
    ...
    if (count === 0) {
      ...
      // Mittelwert der gültigen Subzonen
      leftRaw.push(round1(sumLeft  / count));
      rightRaw.push(round1(sumRight / count));
    }
  }
  ...
  return {
    ...
    right:            rightSmoothed,
    leftUnweighted:   leftRaw,        // Rohwerte nach Supersampling, vor Glättung
...
async function processFont(fontPath) {
  ...
  for (const keyA of glyphKeys) {
    ...
    for (const keyB of glyphKeys) {
      ...
      // Mindestabstand-Capping: nach Korrektur muss jede Zone ≥ baseValue/MINGAP_DIV bleiben
      const minGap = baseValue / MINGAP_DIV;
...
async function main() {
  ...
  console.log(`Font-Margins, Pair-Gaps & Kerning`);
  console.log(`  zones=${ZONES}  smooth=${Math.round(SMOOTH_PCT*100)}%  supersample=${SUPERSAMPLE}  round=${KERN_ROUND}  mingap=1/${MINGAP_DIV}  fonts=${fontFiles.length}\n`);
  ...
  for (const fontFile of fontFiles) {
    ...
    try {
      ...
      if (kerningLines) {
        const csvContent =
          `# Kerning | Font: ${baseName} | zones=${ZONES} supersample=${SUPERSAMPLE} smooth=${Math.round(SMOOTH_PCT*100)}% round=${KERN_ROUND} mingap=1/${MINGAP_DIV}\n` +
